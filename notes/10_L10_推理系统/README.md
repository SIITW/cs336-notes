...
<img src="figures/cover.jpg" style="width:90.0%;height:46.0%" alt="image" />
...
**主讲：**Percy Liang\
**频道：**Stanford Online\
**原课程页面：**[youtube.com/watch?v=EfM546A79aM](https://www.youtube.com/watch?v=EfM546A79aM)\
...
读完后，你应能用 Roofline 模型解释 prefill 与 decode 的不同瓶颈；独立核算参数与 KV cache 的内存账；比较 MQA、GQA、MLA、CLA 和局部注意力；理解量化、剪枝与投机采样的适用边界；并能解释 continuous batching 与 PagedAttention 为什么能把动态请求变成高效服务。
...
![image](figures/inference-schema.png) *模型和 prompt 进入推理过程，产生 response；后文会把中间过程展开为 prefill、KV cache 和逐 token decode。*
...
![image](figures/metrics-overview.jpg) *同一页依次给出 TTFT、单请求 latency 与多请求 throughput。*
...
![image](figures/transformer-shapes.jpg) *左侧直接标出 Q/K/V 与 attention 中的张量形状，右侧定义 $`B,L,T,S,V,D,F,H,N,K,G`$。*
...
![image](figures/kv-prefill-decode.jpg) *红框 prefill 一次填充 KV cache，蓝框 generation 每步追加新 K/V；底部明确对比“可并行的 prefill”与“串行的 generation”。*
...
![image](figures/attention-private-kv.jpg) *上方调试器给出 $`ST/(S+T)`$、$`S/2`$ 与 $`S/(S+1)`$；背景的三条生成链各有自己的 KV cache，说明 batch 不会形成跨请求 KV 复用。*
...
![image](figures/batch-performance-stats.jpg) *课程调试器同时显示三个 batch 的参数量、总内存、latency 与 throughput；$`B=256`$ 的 240.78GB 超过 80GB H100。*
...
![image](figures/gqa-speed.png) *横轴 groups 由 1 增至 64，GQA 从接近 MQA 逐渐回到 MHA；组数更少意味着更多 query heads 共享 K/V。*
...
![image](figures/gqa-accuracy.png) *在这组论文实验中，GQA-8-XXL 的平均质量 47.1 接近 MHA-XXL 的 47.2，而推理时间 0.28s 接近 MQA-XXL 的 0.24s。*
...
![image](figures/mla-schema.png) *斜线填充表示推理时真正缓存的部分。MLA 只缓存右侧 compressed latent，再投影为多头 K/V。*
...
![image](figures/mla-accuracy.png) *DeepSeek-V2 的 Dense 7B 对比中，MHA 在 BBH、MMLU、C-Eval 与 CMMLU 上明显高于 GQA/MQA，说明共享 K/V 的质量结论依赖具体设置。*

再看 MLA 的结果：

![image](figures/mla-accuracy2.png) *Small/Large MoE 的 cache/token 分别从 110.6K/860.2K 元素降到 15.6K/34.6K；多数指标略升，但并非每个单项都更高。*
...
![image](figures/cla-diagram.png) *传统结构每层各自产生 K/V；CLA 的上层通过红色连线复用下层 K/V。*

![image](figures/cla-results.png) *横轴是每 token 的 16-bit KV cache 字节数，纵轴是 validation perplexity，越左下越好；红色 CLA2 点把前沿推向更小 cache。*
...
![image](figures/longformer-attention.png) *依次为 full、sliding window、dilated sliding window、global+sliding window；稀疏 attention 不只有一种固定图案。*
...
![image](figures/deepseek-v4-attention.png) *图中可直接确认 compressor、lightning indexer、Top-k、sliding-window entries、concatenation 和 shared-KV MQA 的数据流。*

字幕可以明确对应：CSA 把每 $`m`$ 个 token 压成一个表示，DSA 从压缩条目中选 Top-k；讲者对 HCA 只说“进一步压缩”，没有把它严谨映射到图中某一独立模块，因此本文不自行指定。

把这条路线放回 5.1 的地图：它同时动了三个轴——compressor 压历史数量（$`S\to S/m`$）、Top-k 进一步压实际参与读取的条目数、sliding window 保证近处细节，而 shared-KV 则沿 head 轴压缩。它的解码成本结构变成了“便宜的索引打分（全历史）+ 昂贵的精确 attention（少数条目）”，这是稀疏注意力共同的会计方式：用一个粗略的全局扫描，换取精细模块只处理少量候选。
...
![image](figures/awq-schema.png) *左为直接 RTN；中间保留 1% FP16 虽改善 PPL，却被标为 bad hardware efficiency；右侧真正的 AWQ 通过 activation-aware scaling 后统一量化为 INT3。*
...
图中 RTN 的 PPL 为 43.2，混合精度和 scale-before-quantize 两种方案都到 13.0，但后者保留规则 INT3 布局。课程报告 fp16→int3 约 4× 更低内存、3.2× 加速；这是特定论文实验，不是所有模型和硬件的固定保证——加速比取决于 kernel 是否真能用 int3 权重做高效反量化矩阵乘，内存比例则是位宽比的直接结果（$`16/3\approx5.3`$，计入 scale/zero point 元数据后约 4×）。
...
![image](figures/pruning-kd-loop.png) *完整流程是 trained LLM → estimate importance → rank → trim → distillation，并可迭代执行。*
...
![image](figures/pruning-kd.png) *横轴是训练模型所用 token 成本，纵轴是 MMLU；图中的“40× cheaper, 9% better”比较的是得到小模型的训练路线，不是宣称单 token 推理 40×。*
...
![image](figures/speculative-decoding-flow.jpg) *上方 target 每次只前进一个 token；下方小 draft model 先提出一串候选，再由大 target model 一次验证后成批接受。*
...
![image](figures/speculative-sampling-algorithm.png) *论文伪代码包含 $`K+1`$ 组 target logits、接受概率 $`\min(1,q/p)`$、拒绝后的正残差，以及全接受后的额外 token。*
...
![image](figures/speculative-sampling-results.png) *在 batch 1、$`K=4`$ 的 Chinchilla 实验里，XSum 约 1.92×/2.01×，HumanEval 约 2.46×，任务指标基本持平或小幅波动。*
...
![image](figures/speculative-sampling-stats.png) *$`K`$ 增大时 target 每轮验证成本上升、接受率下降；XSum 在图中约 $`K=3`$ 最低，HumanEval 到 $`K=7`$ 仍下降，说明甜点区任务相关。*
...
![image](figures/medusa-eagle.png) *Medusa 用多个 heads 并行提出未来候选；EAGLE 让 draft 利用 target 的高层特征，使候选更贴近 target。*

课程仅快速点名这两条扩展路线，没有展开训练目标和候选树验证细节。它们最适合作为“draft 仍有巨大设计空间”的拓展，而不是冒充本课已经完整讲解的算法。用 7.4 的模型看，两者都在优化加速比公式的不同因子：Medusa 消掉独立 draft 模型的串行前向（降低 $`K t_d`$），EAGLE 通过共享 target 特征提高候选质量（提高 $`\alpha`$）。
...
![image](figures/continuous-batching.jpg) *左侧静态 batch 在部分序列结束后留下空槽，右侧按 iteration 调度并把新请求接入。*
...
![image](figures/selective-batching.jpg) *不同长度序列的 attention 分开处理，非 attention 计算拼成 $`[3{+}9{+}5,H]`$。*
...
![image](figures/paged-attention-fragmentation.png) *Request A/B 分别留下 2038、507 个 never-used slots，请求之间还存在 external fragmentation。*
...
![image](figures/paged-attention-blocks.png) *query `forth` 需要从 Block 0、1、2 中取得历史 key/value；块化不会改变逻辑 attention。*

![image](figures/paged-attention-logical.png) *Request A/B 的逻辑顺序保持连续，但物理块可以落在 Block 1/2/3/5/7 等不相邻位置。*
...
![image](figures/paged-attention-sharing.png) *两条翻译请求共享 instruction 和三条示例，只在 task input/output 分叉。*
...
![image](figures/paged-attention-parallel.png) *两个 sample 共享 `Four score and seven / years ago our`，最后分叉成 `fathers` 与 `mothers` 时复制块，引用计数从 2 降为 1。*
